...
ylim([0, yL]); 
hold on 
x_I = 50; y_I = 200; % set initial point
x_G = 600; y_G = 700; % set target point
...
for iter = 1:600
 ...
 x_rand = [unifrnd( X_near(1),x_G),unifrnd( X_near(2), y_G)]; % Generate random points (x,y)
...
x_near(1) = T.v(minIndex).x; % Find the node closest to x_rand in the current tree
 ...
 pHandleList = [pHandleList p_handle]; % The handle index of the drawing is 
count
...
for j = 2:length(path.pos)
res_handle = plot([path.pos(j).x; path.pos(j-1).x;], [path.pos(j).y; 
path.pos(j-1).y], 'green', 'Linewidth', 4);
resHandleList = [resHandleList res_handle];
end
